...
globals [
  ...
  Final-Cost                   ;; variable for the A* algorithm
  valid-patches                ;; variable for the A* algorithm
]
...
turtles-own [
  ...
  chosen-exit                  ;; the exit the agent will choose to evacaute through [exit-north1, exit-north2, exit-south, exit-east]
  ...
  current-path                         ;; an agent's own calculated path, via A* algorithm
]


patches-own [
  parent                       ;;; variable for the A* algorithm
  Cost-path                    ;;; variable for the A* algorithm
  visited?                     ;;; variable for the A* algorithm
  active?                      ;;; variable for the A* algorithm
]
...
to setup
  ...
  setupMap                      ;; setup the floor plan = part of the environment --> see utilities file, make sure to do this first, because for example the colours might be not perfectly
                                ;;white and black, so they are set to perfectly white and black for the code below to work
  set valid-patches patches with [pcolor = white OR pcolor = red]                       ;; set the valid patches to all of those that are white = to choose from in the A* algorithm,
                                                                                        ;; also add the red ones which are the exits
...
to get-paths
  ask visitors [
    set current-path A* patch-here current-destination neighbors with [member? self valid-patches] ;; call function A* with Start end-patch and valid_patches as input. This sets us on a path to our current destination
  ]
end
...
to choose-exit
  set chosen-exit one-of exit-north1      ;; setting the exit choice to exit-north1
                                          ;ifelse familiar-with-exits? 1           ;; ifelse loop
                                          ;[ set chosen-exit .... ]                ;; if the agent is familiar with the environment, choose the nearest exit as my-exit-choice
                                          ;[ set chosen-exit .... ]                ;; else, choose main entrance as exit
end

to move                                   ;;turtle procedure
  if-else  evacuating?                    ;; are we evacuating ?
    [ set current-destination chosen-exit ] ;; if agent is evacauting, change destination to "chosen exit"
  [ if patch-here = current-destination   ;; else (agent is not evacuating), if agent is already at the current-destination, get a new current-destination
    [set current-destination one-of patches with [pcolor = white] ;;setting the new current-destination to a white patch
      set current-path A* patch-here current-destination valid-patches  ;; call function A* with Start end-patch and valid_patches as input
    ]
  ]

  ;take a step on our current path
  let next-patch first current-path
  move-to next-patch
  set current-path butfirst current-path
